      ...
      <div className="app-space-mobile-header md:hidden">
        <div className="relative flex items-center gap-3 overflow-hidden rounded-2xl border border-primary/25 bg-white px-4 py-4 shadow-[0_6px_28px_-8px_rgba(15,23,42,0.30)] backdrop-blur">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-[0.6rem] font-semibold uppercase tracking-[0.08em] text-muted-foreground/70">Espace Guichetière</p>
            <p className="truncate text-sm font-bold text-foreground leading-tight">{guichetiereInfo?.nomComplet}</p>
          </div>
          <NotificationBell
                ...
                {menuItems.map((item) => {
                  ...
                  return (
                    <Link
                      key={item.path}
                      to={item.path}
                      className={`flex items-center gap-2.5 rounded-2xl border border-transparent px-3 py-2.5 text-sm font-semibold transition-all ${
                        isActive
